...
たとえば、[注釈: 第二法則熱力学] または [注釈: エントロピー増加] は、多くの物理システムが不可逆的によりカオスな状態に向かう傾向があることを説明しています。この章では、この動作の性質が単純なプログラムで簡単に観察できることを明らかにします。簡単なプログラムのコンテキストで第 2 法則を研究することにより、それが一般的に成り立つ理由だけでなく、それが失敗する条件についても明確に理解することができます。
...
このアイデアは、[Wolfram Physics Project](注釈: wolfram-physics-project) などの研究プログラムによって継続されています。
...
おそらく、最も低いレベルでは、宇宙はノードから構成される巨大な [ネットワーク システム](注釈: ネットワーク システム) です。このネットワークでは、ノードには固有の位置がありません。定義されているのは、他のノードとの接続関係だけです。ノードの数が十分に多い場合、そこからよく知られた空間特性やその他の物理現象が現れることがあります。
...
究極の物理理論を確立するには、空間の性質だけでなく時間の性質も理解する必要があります。日常の経験から、空間と時間は大きく異なることがわかりますが、基礎物理学のほとんどの現代モデルは、それらは本質的に同じであるという考え、いわゆる「[注釈: 時空連続体]」に基づいています。
...
セル オートマトンの特徴は、すべてのセルが同期して更新されることであり、これにはグローバル クロックが必要です。しかし、このような地球時計が宇宙に存在するのは無理があるように思えます。考えられる代替案の 1 つは、ユニバースがアノテーションのように動作することです。つまり、各ステップで 1 つのアクティブ セルだけが更新されるモバイル オートマトンまたはチューリング マシンです。
...
「更新イベント」とその因果関係のネットワークを構築することで、これをより深く理解できます。この「[因果ネットワーク](注釈:因果ネットワーク)」は、内部観察者が知覚する時空間構造を定義します。このネットワークでは、接続は一方向であり、あるイベントから別のイベントへの因果関係を表し、意味のある時間の概念につながります。
...
モバイル オートマトン モデルは、単一のアクティブ セルを通じてイベントのシリアル化を強制します。ただし、より一般的な注釈:substitution-systems (置換システム) では、各ステップで複数の置換が可能です。重要な問題は、置換の順序が最終的な因果ネットワークに影響を与えるかどうかです。
...
宇宙がノードの広大なネットワークである場合、それはどのように進化するのでしょうか?最も簡単な方法は、[Substitution Systems](annotation:substitution-systems) のように、ネットワークの特定の部分を固定ルールに従って置き換えることです。最も単純なルールは、各ノードがノードの固定クラスターに置き換えられるというものですが、これでは規則性の高い入れ子構造のみが生成されます。
...
システムの基礎となるルールが「[注釈:因果不変]」(因果不変性) を持っている場合、つまり、適用される順序に関係なく、同じ因果ネットワークを生成する場合、重要な結論が得られます。物理法則は、一定の速度で移動するすべての観測者にとって同じです。これが特殊相対性理論の中心原理です。
...
粒子は、空間を表す広大でランダムなネットワークの中でその特性を維持する永続的な構造です。電荷などの特定の保存量は、ルールによって保護されているネットワーク内の特定のローカル トポロジ (注釈:空間もつれ) に関連している可能性があります。さまざまな粒子タイプが、個別の「コア」構造の可能性のある小さなセットに対応します。

粒子の動きは、因果ネットワークにおける一連の基礎的な更新ステップの結果です。因果的不変性により、パーティクルはさまざまな速度で安定して移動できます。粒子のエネルギーと運動量は、粒子のコアの周囲のノードの数に関連している可能性があります。

## 重力現象

大規模なスケールでは、最も重大な影響は重力です。一般相対性理論は重力を[注釈: 空間曲率]に関連付けます。離散ネットワークでは、曲率は接続パターンの変化として理解できます。たとえば、二次元平面六角形ネットワークに五角形を導入すると、(球のような) 正の曲率が生じますが、七角形を導入すると、負の曲率が生じます。
...
私は、この章で説明する種類のプログラムは、最終的にはアノテーション、つまり量子論の主要な機能の多くまたはすべてを示すことが証明されると強く信じています。
...
注釈の違反であるベル不等式など、量子論の多くのユニークな特徴は、通常の空間距離を超えた粒子間のつながりを示唆しているようです。ネットワーク モデルでは、これは次のように説明できます。通常の空間では粒子が遠く離れていても、中間の空間ノードを通過しないいくつかの「[キュー](アノテーション:スレッド)」(スレッド)を介して直接接続することができます。
