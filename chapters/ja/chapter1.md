# [第 1 章: 新しい科学の基礎](annotation:chapter1)
...
このことから、私は [計算等価性の原理](annotation:computational-equivalence) を提案しました。動作が単純でなくなる限り、その計算の複雑さは同等です。これは、単純なプログラムが、私たちには非常に複雑に見える現象を示すことができる理由を説明します。また、多くのシステムは [計算上既約](annotation:computational-irreducibility) であり、数式を使用して推定することができないことも意味します。唯一の方法は、それらを段階的に計算することです。
...
この複雑さの結果の 1 つは、伝統的な数学の中心であった証明の概念に根本的な制限が生じることです。 1930 年代にはすでに、[ゲーデルの定理](annotation:godel-theorem) がこの限界をある程度示唆していました。しかし以前は、それらは数学のほとんどの実際的な応用には無関係であるように思われていました。しかし、この本の調査結果は、これが現在数学と考えられているものの範囲がいかに小さいかを主に反映していることを示しています。実際、この本の核心は、新しいアイデアや方法、そして探求すべき広大な新しい分野とともに、数学の主要な一般化の紹介であると見ることができます。
...
### 物理学
<div class="content-layer simplified">
単純なプログラムの発見により、[熱力学的挙動](annotation:thermodynamics) が物理学でどのように生じるのかが直接説明され、1 世紀以上にわたる混乱が解決されました。連続数や [微積分](annotation:calculus) に依存する従来の方法とは対照的に、単純なプログラムは、これまで謎だと考えられていた物理現象の基本的なメカニズムを捉えることができます。この本は、宇宙の真の基本理論を発見するための重大な可能性を提供します。
</div>
<div class="content-layer detailed">
...
<div class="content-layer detailed">
自然は、いとも簡単に素晴らしい美しさを生み出すようです。しかし、かつては、芸術は主にこれらの形式を模倣することで満足していました。しかし今では、単純なプログラムが自然界のさまざまな複雑な動作の基本的なメカニズムを捉えることができるという発見により、これらのプログラムをサンプリングするだけで、私たちが目にする自然の形態への一般化を探求することを想像できるようになりました。従来の科学的直観、そして初期の [コンピューター アート](annotation:computational-art) では、単純なプログラムでは常に単純すぎて堅苦しく芸術的に面白くない画像が生成されると考える人がいるかもしれません。しかし、この本を読み進めると、ルールが非常に単純なプログラムであっても、しばしば自然を彷彿とさせる、しかし多くの場合これまでに見たものとは異なる、驚くほど美しい品質の画像を生成できることが明らかになります。
</div>
<button class="expand-toggle" data-target="simplified" data-expanded="false">
  <span class="toggle-text">詳細を展開</span>
  <span class="toggle-icon">▼</span>
...
### [大惨事理論](annotation:catastrophe-theory)
...
### [カオス理論](annotation:deterministic-chaos)
<div class="content-layer simplified">
...
### [計算複雑性理論](annotation:computational-complexity)
...
線形数学方程式は通常、非常に簡単に解くことができ、科学で広く使用されています。非線形分野は、より複雑な方程式の解析に重点を置いています。最も成功したものは [ソリトン方程式](annotation:soliton-equations) で、慎重な操作によって線形のような特性が得られます。しかし、この本で説明されている種類のシステムは複雑な動作を示すことが多く、そのような単純化する特性はありません。
...
### [自己組織化](annotation:self-organization)
...
<div class="content-layer simplified">
統計力学は、個々のケースの複雑さを隠すために平均を使用し、その基礎が長い間疑問視されてきた [エントロピー増大の原理](annotation:entropy-increase) に依存しています。本書は、その基礎を明らかにすることが期待される新しい計算フレームワークを提供します。
</div>
<div class="content-layer detailed">
統計力学は 1 世紀前に発展した物理学の分野で、多数の気体分子やその他の部品で構成されるシステムの平均的な挙動を理解することに関係しています。どのような場合でも、このようなシステムは複雑な動作をします。しかし、統計力学では、多数のインスタンスの平均を調べることで、この複雑さを回避することがよくあります。現実と接触するには、通常、いわゆる熱力学の第 2 法則、または [エントロピー増大の原理](annotation:entropy-increase) が使用されます。しかし、1 世紀以上もこの原理に依存していると、この原理を理解するのには常に理解できない困難が伴います。この本のアイデアを使えば、これらの問題に対処できるフレームワークができると思います。
</div>
<button class="expand-toggle" data-target="simplified" data-expanded="false">
  <span class="toggle-text">詳細を展開</span>
  <span class="toggle-icon">▼</span>
...
しかし、私がこの本で検討しているシステムの主要なクラスの特殊なケース、[セル オートマトン](annotation:cellular-automata) に取り組んでいたことが判明しました。シミュレーションを物理的に可能な限り現実的にしたいという私の願望から生じる主に技術的な問題がなければ、おそらく 1974 年までに、私がこの本で説明している主要な現象のいくつかを発見していただろう。

しかし、その時私は、当時科学の最も基本的な分野であると思われた理論 [素粒子物理学](annotation:particle-physics) にエネルギーを捧げることに決めました。その後数年間で、私は素粒子物理学と宇宙論のいくつかの分野で大きな進歩を遂げました。しかし、しばらくすると、私が遭遇した最も重要かつ根本的な問題の多くは、これらの分野の難解な詳細とは全く独立しているのではないかと疑い始めました。

実際、日常の一般的な現象に関する多くの関連する質問でさえ、まったく答えられていないことに気づきました。たとえば、[乱気流](annotation:turbulence) で見られる複雑なパターンの根本的な起源は何でしょうか?雪の結晶の複雑な模様はどのようにして作られるのでしょうか？植物や動物がこれほど複雑な方法で成長することを可能にする基本的なメカニズムは何ですか?
...
しかし、これほど根本的なことがこれまで気づかれなかったということがどうしてあり得るでしょうか?私は科学文献を検索し、多くの人々と話をし、私が取り組んでいたものと同様のシステムが 30 年前に「[セルラー オートマトン](annotation:cellular-automata)」と名付けられていたことを発見しました。しかし、何度か近い試みをしたにもかかわらず、私がやったような実験を実際に試した人は誰もいませんでした。

しかし、私は、私が見ていた基本的な現象が、何らかの既知の科学原理の明白な結果であるに違いないという懐疑的なままでした。ただし、カオス理論や [フラクタル幾何学](annotation:fractals) などの分野のアイデアは、特定の機能を説明するのに役立つとは思いますが、現象全体に近いものはこれまで研究されたことがないようです。

[セルオートマトン](annotation:cellular-automata) の動作に関する私の初期の発見は、科学コミュニティでのかなりの活動に影響を与えました。 1980 年代半ばまでに、物理学、生物学、コンピューター サイエンス、数学、その他の分野で多くの応用が発見されました。実際、私が発見した現象のいくつかは、私が複雑系理論と呼ぶ新しい研究分野の基礎として使用され始めました。
...
1980 年代初頭に [セル オートマトン](annotation:cellular-automata) について初めて発見したとき、私は何か重要な出来事の始まりを見ているのではないかと疑いました。しかし、それが最終的にどれほど重要になるかはわかりませんでした。実際、過去 20 年間で、私は想像していたよりもはるかに多くのことを発見しました。私が構築に多大なエネルギーを費やした新しい科学は、将来の知識の発展にとってますます中心的かつ重要な方向性となっているようです。
</div>
<button class="expand-toggle" data-target="simplified" data-expanded="false">
  <span class="toggle-text">詳細を展開</span>
  <span class="toggle-icon">▼</span>
